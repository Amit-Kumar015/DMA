  ...
  const GetSponser = () => {
    ...
    const handleSubmit = async () => {
        try {
          ...
          if (!selectedRoleId || !selectedSponsor?.value || !selectedProduct?.value) {
            Alert.alert('Error', 'Please select all required fields.');
            return;
          }
      
          const response = await axios.get('http://52.70.194.52/api/core/give/sponsor-users/', {
            params: {
              i_am: selectedRoleId,
              collaboration_type: selectedSponsor.value, // 👈 pass value string like "paid"
              product_type: selectedProduct.value,       // 👈 pass value string like "supplements, clothing"
            },
            headers: {
              Authorization: `Bearer ${accessToken}`,
            },
          ...
          console.log('📦 Final Params:', {
            i_am: selectedRoleId,
            collaboration_type: selectedSponsor.value,
            product_type: selectedProduct.value,
          });
          console.log('📥 Sponsor Users Response:', response);
          // 🔄 handle the fetched data here
        } catch (error) {
          console.error('❌ Fetch Sponsor Users Error:', error.response?.data || error.message);
          Alert.alert('Error', error.response?.data?.message || 'Failed to fetch sponsor users.');
        }
      };
