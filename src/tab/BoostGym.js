...
const [selectedLocation, setSelectedLocationDetails] = useState({
  ...
  try {
    ...
    const response = await axios.get('http://52.70.194.52/api/core/subsubcategories/', {
      headers: {
...
const filterData = (text) => {
  const filtered = interestData.filter((item) =>
    item.toLowerCase().includes(text)  // Correct usage for string filtering
  );
...
const handleSelectionChange = (uniqueId, selectedOption) => {
  ...
  {step === 0 && <Header showBack={true} title="Boost Gym" />}
 ...
 <View style={{ }}>
    ...
    {step === 0 && (
      <>
        ...
        <ButtonWithPushBack customContainerStyle={styles.buttonContainer}>
          <PrimaryButton title="Next" onPress={() => setStep(1)} />
        </ButtonWithPushBack>
      </>
    )}
  </KeyboardAwareScrollView>
  ...
  {step === 1 && (
  <Slide index={1}>
    <Header showBack={true} title="Boost Academy" customBackEvent={() => setStep(0)} />
  ...
  <View style={{ width: '48%' }}>
    <SingleSelect
      arrayData={Array.from({ length: 18 }, (_, i) => ({ key: `${i + 1}`, value: `${i + 1}` }))}
      selected={minAge}
      selectedCb={handleSelectionChange}
      uniqueId="minAge"
  ...
  <View style={{ width: '48%' }}>
    <SingleSelect
      arrayData={Array.from({ length: 83 }, (_, i) => ({ key: `${i + 18}`, value: `${i + 18}` }))}
      selected={maxAge}
      selectedCb={handleSelectionChange}
      uniqueId="maxAge"
...
{locations.length > 0 && location.length >= 3 && (
 ...
 {step === 2 && (
  <Slide index={2}>
    <Header showBack={true} title="Boost Academy" customBackEvent={() => setStep(1)} />
